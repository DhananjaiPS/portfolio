...
import { motion, AnimatePresence } from 'framer-motion';
import { School, BookOpen, GraduationCap, Star, Navigation, Rocket, MapPin } from 'lucide-react';
import { Canvas, useFrame, useThree } from '@react-three/fiber';
import { Float, MeshDistortMaterial, Stars, Sparkles as DreiSparkles, Line } from '@react-three/drei';
import * as THREE from 'three';
...
const schools = [
  {
    id: 1,
    name: 'Primary Airforce School, J&K',
    period: 'Primary Class',
    details: 'Started my foundational years here. Built my core discipline and learned the absolute basics of curiosity!',
    icon: School,
    image: "https://www.airforceschoolsrinagar.com/files/photo%205.jpg",
    color: 'from-orange-400 to-amber-500',
    yOffset: 0
  },
  {
    id: 2,
    name: 'Kendriya Vidyalaya No.2 Srinagar, J&K',
    period: '1st Class',
    details: 'Spent my early school days surrounded by the breathtaking mountains of Srinagar, building a strong academic base.',
    icon: School,
    image: "https://yayskool-school-images.s3.ap-south-1.amazonaws.com/kendriya-vidyalaya-no-2-srinagar-579317163.png",
    color: 'from-blue-300 to-blue-500',
    yOffset: 66
  },
  {
    id: 3,
    name: 'Kendriya Vidyalaya Jalipa Cantt, Barmer, Rajasthan',
    period: '2nd-5th Class',
    details: 'Adapted to the desert heat! Thrived in extracurriculars and started developing a fiercely competitive spirit on the playground.',
    icon: School,
    image: '/kvja.jpg',
  ...
  {
    id: 4,
    name: 'Kendriya Vidyalaya (KV) STC BSF Humhama, J&K',
    period: '5th-7th Class',
    details: 'Returned to J&K. This is where athletics took over—I dominated the ground as a relentless Kabaddi and Kho-Kho player!',
    icon: School,
    image: '/kvstc.jpg',
  ...
  {
    id: 5,
    name: 'Kendriya Vidyalaya Simultaia, Krishnanagar, Nadia, West Bengal',
    period: '7th-9th Class',
    details: 'Moved to West Bengal. Balanced my academics with intense sports training, proudly stepping up to take on the role of House Captain.',
    icon: BookOpen,
    image: 'https://yayskool-school-images.s3.ap-south-1.amazonaws.com/kendriya-vidyalaya-nadia-835311690.png',
    color: 'from-teal-400 to-cyan-500',
    yOffset: -48
  },
  {
    id: 6,
    name: 'Kendriya Vidyalaya No. 2 Kanchrapara, 24 Parganas (N), WB',
    period: '9th-12th Class',
    details: 'Served as Sports Captain while crushing academics (93% in 12th PCM). Post-11th class, I wrote my very first lines of code—changing my trajectory forever.',
    icon: Star,
    image: 'https://lh3.googleusercontent.com/gps-cs-s/AHVAwepScTjOGUAoaXt3QKX9SBSxPe2LiL83pKTpe5fndIcJHg6ueCSOQGW2kV7YXE2GJfhWYUdQE3itH3Thtnwm0Mq4ppeqH8PS4vA2PrIeRWWxMXbWR8fr3qgrNFfAIDLMWfCzEm4=s1360-w1360-h1020-rw',
    color: 'from-cyan-400 to-sky-500',
    yOffset: 34
  },
  {
    id: 7,
    name: 'Moradabad Institute of Technology, Moradabad, Uttar Pradesh',
    full_name: 'Dr. A.P.J. Abdul Kalam Technical University',
    period: 'B.Tech CSE (IoT)',
    details: 'Merged my athletic discipline with hardcore software development. Mastered Full Stack & IoT hardware. Current CGPA: 8.5.',
    icon: GraduationCap,
    image: '/mit.jpeg',
...
// --- 3D SCENE LOGIC (Kept mostly intact, just amped up the colors) ---
function createCurve() {
  return new THREE.CatmullRomCurve3([
    new THREE.Vector3(-7, -1.1, 0),
    new THREE.Vector3(-5.3, 1.2, -1),
...
function Traveler({ activeIndex, curve }: { activeIndex: number; curve: THREE.CatmullRomCurve3 }) {
  ...
  return (
    <group ref={groupRef} scale={0.85}>
      <mesh ref={orbRef}>
        <icosahedronGeometry args={[0.75, 1]} />
        <meshStandardMaterial color="#ff2a7a" emissive="#e11d48" emissiveIntensity={1} wireframe />
      </mesh>
      <mesh position={[0, -0.95, 0]}>
        <sphereGeometry args={[0.24, 20, 20]} />
        <meshStandardMaterial color="#fde68a" emissive="#f59e0b" emissiveIntensity={0.9} />
      </mesh>
      <DreiSparkles count={20} scale={4} size={3} speed={0.8} color="#00ffcc" />
    </group>
  );
}
...
function Scene({ activeIndex }: { activeIndex: number }) {
  ...
  return (
    <>
      ...
      <directionalLight position={[10, 10, 5]} intensity={1.4} color="#fef08a" />
      <directionalLight position={[-10, -10, -5]} intensity={0.8} color="#ff90e8" />
      ...
      <Float speed={3} rotationIntensity={2} floatIntensity={3}>
        <group position={[0, 0.4, -4]} scale={2.1}>
          ...
          <mesh>
            <octahedronGeometry args={[1.05]} />
            <MeshDistortMaterial color="#38bdf8" distort={0.4} speed={3} wireframe />
          </mesh>
        </group>
      </Float>

      <Line points={curve.getPoints(180)} color="#1e293b" lineWidth={5} dashed dashScale={1} dashSize={0.22} gapSize={0.14} />
      <Traveler activeIndex={activeIndex} curve={curve} />
      <DreiSparkles count={150} scale={[20, 10, 10]} size={4} speed={0.6} color="#fbbf24" />
    </>
  );
}
...
export default function EducationalMapAdventure() {
  ...
  return (
    <section className="relative min-h-screen overflow-hidden bg-white font-sans selection:bg-pink-400 selection:text-white">

      {/* 3D Canvas Background */}
      <div className="pointer-events-none absolute inset-0 z-0 opacity-40 mix-blend-multiply filter blur-[1px]">
        <Canvas camera={{ position: [0, 1, 10], fov: 45 }}>
          <Scene activeIndex={activeIndex} />
        </Canvas>
      </div>
      ...
      <div
        ...
        style={{ backgroundImage: 'radial-gradient(#1e293b 2px, transparent 2px)', backgroundSize: '30px 30px' }}
      />

      <div className="container relative z-10 mx-auto flex h-full flex-col px-4 md:px-6 pt-12 pb-24">

        {/* HEADER SECTION */}
        <motion.div initial={{ opacity: 0, y: -20, rotate: -2 }} animate={{ opacity: 1, y: 0, rotate: 0 }} className="mt-8 mb-12 text-center relative z-20">
          {/* <div className="mb-6 inline-flex items-center gap-2 border-4 border-slate-900 bg-[#a3e635] px-6 py-2 text-[14px] font-black uppercase tracking-[0.2em] text-slate-900 shadow-[4px_4px_0px_#1e293b] rotate-[-2deg] hover:rotate-0 transition-transform cursor-crosshair">
        ...
        {/* TIMELINE NAVIGATION */}
        {/* TIMELINE NAVIGATION */}
        <div className="relative mx-auto mb-16 w-full max-w-6xl px-2">
          {/* py-28 ensures the bouncing animations don't get cut off vertically */}
          <div className="hide-scroll-bar overflow-x-auto py-28">

            <div className="relative flex min-w-[920px] items-center  md:min-w-0 md:justify-between px-4">

              {/* 🔥 FIX: Deleted the ugly thick straight line entirely! */}
              ...
              {schools.map((school, index) => {
                ...
                return (
                  <React.Fragment key={school.id}>
                    ...
                    <button
                      onClick={() => setActiveIndex(index)}
                      className="group relative flex w-[120px] shrink-0 flex-col items-center outline-none"
                      style={{ transform: `translateY(${school.yOffset}px)` }}
                    >
                      {isActive && (
                        <motion.div
                          layoutId="pin"
                        ...
                        >
                          <MapPin size={44} className="text-pink-500 fill-pink-500 stroke-slate-900 stroke-[1.5px] drop-shadow-[4px_4px_0px_rgba(30,41,59,1)] animate-bounce" />
                        </motion.div>
                      )}
                      ...
                      <motion.div
                        ...
                        whileTap={{ scale: 0.95 }}
                        className={`relative z-20 flex w-full flex-col items-center justify-center border-4 border-slate-900 px-3 py-3 text-center transition-all duration-200 ${isActive
                          ? `${school.color} shadow-[8px_8px_0px_#1e293b] scale-110 -translate-y-2`
                          : 'bg-white shadow-[4px_4px_0px_#1e293b] hover:shadow-[6px_6px_0px_#1e293b]'
                          }`}
                      >
                        <h3 className="line-clamp-2 text-[12px] font-black uppercase leading-tight text-slate-900">
                          {school.name}
                        </h3>
                        <div className="mt-2 border-t-2 border-slate-900 pt-1 w-full text-[10px] font-bold text-slate-900">
                          {school.period}
                        </div>
                      </motion.div>
                    </button>

                    {/* 🔥 FIX: Added Neo-Brutalist Dashed Arrows between items */}
                    {index !== schools.length - 1 && (
                      <div className="shrink-0 text-slate-300 drop-shadow-sm">
        ...
        <div className="mx-auto w-full max-w-5xl px-2 relative z-30">
          <AnimatePresence mode="wait">
            <motion.div
              key={activeIndex}
              ...
              className={`relative flex flex-col gap-0 border-4 border-slate-900 ${schools[activeIndex].color} shadow-[16px_16px_0px_#1e293b] md:flex-row`}
            >
              {/* Masking Tape */}
              <div className="absolute -top-4 left-1/2 -translate-x-1/2 w-32 h-8 bg-yellow-100 border-2 border-slate-400 rotate-2 z-40 opacity-90 shadow-sm" />

              {/* Left Image Section */}
              <div className="w-full shrink-0 border-b-4 border-slate-900 bg-[#fffdf0] md:w-2/5 md:border-b-0 md:border-r-4 p-4 sm:p-6">
                <div className="relative h-48 w-full overflow-hidden border-4 border-slate-900 shadow-[6px_6px_0px_#1e293b] md:h-full group bg-slate-200">
                  <img
                    src={schools[activeIndex].image}
                    alt={schools[activeIndex].name}
                    className="h-full w-full object-cover grayscale-[40%] contrast-125 transition-all duration-500 group-hover:grayscale-0 group-hover:scale-105"
                  />
                  {/* Photo Corner Tags */}
                  <div className="absolute top-2 left-2 bg-yellow-300 border-2 border-slate-900 px-2 py-1 text-xs font-black uppercase shadow-[2px_2px_0px_#1e293b] rotate-[-5deg]">
                    Snapshot
                  </div>
                </div>
              </div>
              ...
              <div className="flex h-full flex-col justify-center bg-[#fffdf0] p-6 sm:p-10 w-full relative overflow-hidden">
                {/* Halftone background pattern */}
                <div className="absolute inset-0 opacity-[0.03] pointer-events-none bg-[radial-gradient(circle_at_center,black_1px,transparent_1px)] bg-[length:4px_4px]" />

                <div className="relative z-10">
                  <div className="mb-6 flex items-center gap-4">
                    <div className={`flex h-14 w-14 items-center justify-center border-4 border-slate-900 ${schools[activeIndex].color} shadow-[4px_4px_0px_#1e293b] rotate-[-3deg]`}>
                      <ActiveIcon size={28} strokeWidth={2.5} className="text-slate-900" />
                    </div>
                    <div className="border-4 border-slate-900 bg-slate-900 px-4 py-1 text-[14px] font-black uppercase text-white shadow-[4px_4px_0px_#38bdf8] rotate-2">
                      Stage {activeIndex + 1} / {schools.length}
                    </div>
                  </div>

                  <h3 className="text-3xl sm:text-5xl font-black uppercase leading-none tracking-tighter text-slate-900 mb-2">
                    {schools[activeIndex].name}
                  </h3>

                  {schools[activeIndex].full_name && (
                    <div className="mb-6 inline-block border-b-4 border-pink-400 pb-1">
                  ...
                  <p className="max-w-[50ch] text-[16px] font-bold leading-relaxed text-slate-700 md:text-[18px]">
                    {schools[activeIndex].details}
                  </p>
                </div>
              </div>
      ...
      <style dangerouslySetInnerHTML={{ __html: `.hide-scroll-bar::-webkit-scrollbar { display: none; }` }} />
    </section>
  );
}