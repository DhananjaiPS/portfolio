...
import { GraduationCap, Briefcase, Award, Sparkles, Pin, ArrowRight, Heart, PersonStanding, Earth, Sparkle } from 'lucide-react';
...
const itemVars: Variants = {
    ...
    show: {
        ...
        y: 0,
        filter: 'blur(0px)',
        transition: { duration: 0.5, ease: "easeOut" } // String use karne se koi error nahi aayega
    }
};
...
export default function About() {
    return (
        <section className="relative min-h-screen bg-white py-24 sm:py-32 overflow-hidden font-sans">
            ...
            <motion.div animate={{ rotate: [0, 10, -10, 0] }} transition={{ repeat: Infinity, duration: 5 }} className="absolute top-20 right-10 text-yellow-400 text-5xl z-0">✦</motion.div>
            <motion.div animate={{ y: [0, -15, 0] }} transition={{ repeat: Infinity, duration: 4 }} className="absolute bottom-40 left-10 text-pink-400 text-5xl z-0">❤</motion.div>
            <motion.div animate={{ x: [0, 15, 0] }} transition={{ repeat: Infinity, duration: 6 }} className="absolute top-1/2 right-[5%] text-sky-400 text-6xl z-0 opacity-50">★</motion.div>

            <div className="container relative z-10 mx-auto max-w-7xl px-6 md:px-12">

                <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-20 items-center">

                    {/* LEFT SIDE: Dragable Polaroid Photo */}
                    <motion.div
                    ...
                    >
                        {/* Interactive Polaroid - You can actually DRAG it! */}
                        <motion.div
                            drag
                            dragConstraints={{ left: -20, right: 20, top: -20, bottom: 20 }}
                            whileHover={{ scale: 1.05, rotate: 0 }}
                            whileTap={{ scale: 0.95, rotate: 2, cursor: "grabbing" }}
                            className="relative p-4 pb-16 bg-white border-2 border-slate-800 shadow-[12px_12px_0px_#1e293b] cursor-grab mx-auto max-w-[320px] sm:max-w-md transition-transform duration-300"
                        >
                            {/* Masking Tape */}
                            ...
                            {/* <img
                                ...
                                alt="Me coding"
                                className="w-full aspect-square object-cover border-2 border-slate-800 grayscale-[30%] hover:grayscale-0 transition-all duration-500"
                            /> */}
                            <img
                                src="/pic70.png"
                                alt="Me coding"
                                className="w-full aspect-square object-cover border-2 border-slate-800 
             transition-all duration-500 filter brightness-120 contrast-100 saturate-110
             hover:brightness-125 hover:contrast-100 animate-floating transition-transform duration-700 ease-out"
                            />

                            {/* Hand-written text on Polaroid */}
                            <div className="absolute bottom-4 left-0 w-full text-center font-serif italic font-bold text-slate-800 text-xl flex items-center justify-center gap-2">
                                Just me & my bugs <FaBug />
                            </div>

                            {/* Cute floating sticker on the photo */}
                            <img src="/pic46.jpg" alt="sticker" className="absolute -right-8 -bottom-8 w-29 h-29 rounded-2xl drop-shadow-lg rotate-12 pointer-events-none animate-floating transition-transform duration-700 ease-out" />
                            <img src="/pic48.jpg" alt="sticker" className="absolute -left-8 -top-8 w-29 h-29 rounded-2xl drop-shadow-lg rotate-12 pointer-events-none animate-floating transition-transform duration-700 ease-out" />
                        </motion.div>
                    </motion.div>
                    ...
                    >
                        ...
                        <motion.h2 variants={itemVars} className="mb-8 text-5xl md:text-6xl font-black text-slate-900 tracking-tighter relative inline-block w-max">
                            ...
                            <svg className="absolute w-full h-4 -bottom-1 left-0 text-pink-400" viewBox="0 0 200 20" preserveAspectRatio="none">
